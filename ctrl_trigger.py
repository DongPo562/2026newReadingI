...
功能：
1. 检测纯 Ctrl 键长按 ≥ 1.0 秒（期间无其他按键）则立即启动流程
2. 不需要等 Ctrl 释放，释放后流程继续
3. Ctrl 期间按其他键 → 取消流程
4. 使用 winocr 获取鼠标悬浮位置的单词
5. 清洗文本（去除开头/结尾的 !?.,）
6. 校验文本（只允许 a-zA-Z'- 和空格）
7. 重复文本：等系统声音结束 -> 延迟 -> 播放已有录音
8. 新文本：录制 -> 保存 -> 刷新 UI
9. 新 Ctrl 按下 → 立即取消旧流程
"""
import re
...
class CtrlTriggerListener:
    ...
    def _on_key_press(self, key):
        ...
        if self._is_ctrl_key(key):
            ...
            if self.ctrl_press_time is None:
                ...
                self.other_key_pressed = False
                print("[CtrlTrigger] Ctrl pressed, starting hold timer")
            ...
            if self.ctrl_press_time is not None:
                ...
                # 取消正在运行的流程
                if self.current_thread and self.current_thread.is_alive():
                    print("[CtrlTrigger] Cancelling flow due to other key press")
                    self.cancel_event.set()
    ...
    def _on_key_release(self, key):
        ...
        if self.ctrl_press_time is not None:
            ...
            print(f"[CtrlTrigger] Ctrl released after {held_duration:.2f}s (flow continues if started)")

        self.ctrl_press_time = None
        # 注意：不取消正在运行的流程，让它继续

    def _on_hold_timer_fired(self):
        """长按计时器触发：检查条件并启动流程"""
        self.hold_timer = None

        # 检查条件
        if self.ctrl_press_time is None:
            print("[CtrlTrigger] Hold timer fired but Ctrl already released")
            return

        if self.other_key_pressed:
            print("[CtrlTrigger] Hold timer fired but other key was pressed")
            return

        # 条件满足，启动流程
        print("[CtrlTrigger] Hold duration reached, executing trigger...")
        self._execute_trigger()

    def _execute_trigger(self):
        """执行触发流程"""
        # 如果有旧流程还在运行（理论上应该已经取消了），等待它退出
        if self.current_thread and self.current_thread.is_alive():
            print("[CtrlTrigger] Waiting for previous thread to exit...")
