...
def extract_letter_sequence(text, max_chars=300):
    """
    Extract pure English letter sequence from text.

    Rules:
    - Keep only a-z and A-Z
    - Ignore spaces, punctuation, numbers, newlines
    - Convert to lowercase
    - Only process first max_chars characters

    Args:
        text (str): Input text
        max_chars (int): Maximum characters to process (default 300)

    Returns:
        str: Extracted letter sequence
    """
    if not text:
        return ""
