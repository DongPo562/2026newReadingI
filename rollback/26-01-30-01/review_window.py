...
新增: Alt键模拟功能 - 鼠标悬浮在单词区域时自动按下Alt键
"""
import os
...
class ReviewWindow(QWidget):
    def __init__(self, db_manager, parent=None):
        ...
        # Alt 安全定时器：防止 Alt 键卡住
        self.alt_safety_timer = QTimer()
        self.alt_safety_timer.timeout.connect(self._check_alt_safety)
        self.alt_safety_timer.start(ALT_SAFETY_CHECK_MS)
    ...
    def init_ui(self):
        ...
        # Alt 状态指示按钮
        self.btn_alt_status = QPushButton("Alt")
        self.btn_alt_status.setObjectName("altStatusBtn")
        self.btn_alt_status.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_alt_status.clicked.connect(self._on_alt_button_clicked)
        self.btn_alt_status.setProperty("active", "false")
        self.btn_alt_status.setToolTip("鼠标悬浮单词区时自动按下Alt\n点击可手动释放")
        ...
        row1.addWidget(self.btn_alt_status)  # 添加 Alt 状态按钮
        row1.addStretch()
    ...
    def _press_alt(self):
        ...
        if not self.alt_pressed:
            try:
                ...
                self._update_alt_button(True)
                print("[ReviewWindow] Alt 已按下 (进入单词区)")
            except Exception as e:
                print(f"[ReviewWindow] 按下 Alt 失败: {e}")
    ...
    def _release_alt(self):
        ...
        if self.alt_pressed:
            try:
                ...
                self.alt_pressed = False
                self._update_alt_button(False)
                print("[ReviewWindow] Alt 已释放 (离开单词区)")
            except Exception as e:
                print(f"[ReviewWindow] 释放 Alt 失败: {e}")

    def _update_alt_button(self, active):
        """更新 Alt 状态按钮的显示"""
        self.btn_alt_status.setProperty("active", "true" if active else "false")
        self.btn_alt_status.style().unpolish(self.btn_alt_status)
        self.btn_alt_status.style().polish(self.btn_alt_status)

    def _on_alt_button_clicked(self):
        """点击 Alt 按钮：强制释放 Alt 键"""
        if self.alt_pressed:
            self._release_alt()
            print("[ReviewWindow] Alt 已手动释放")

    def _check_alt_safety(self):
        """安全检查：窗口不可见或鼠标不在单词区时，强制释放 Alt"""
        if self.alt_pressed:
