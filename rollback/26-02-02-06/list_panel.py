...
class AudioListItem(QWidget):
    ...
    def init_ui(self):
        ...
        # self.label.setToolTip(self.content)  # 已禁用 tooltip
...
class ListPanel(QWidget):
    ...
    def on_silent_record_start(self):
        ...
        if not app_config.auto_record_follow_main_autoplay:
            # 配置为不跟随主界面设置，进入静默模式
            self.silent_record_mode = True
            print("[ListPanel] Silent record mode ENABLED (will not auto-play after recording)")
        else:
            # 配置为跟随主界面设置，不进入静默模式
            print("[ListPanel] Silent record mode disabled (will follow main autoplay setting)")
    ...
    def on_auto_play_signal(self, message):
        ...
        # 检查是否处于静默录音模式
        if self.silent_record_mode:
            print("[ListPanel] Silent record mode active, skipping auto-play")
            self.silent_record_mode = False  # 重置静默模式
            return
