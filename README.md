...
- 已支持在阅读场景中按 `Ctrl+U` 触发出题流程（经 FastKeys 映射后可由鼠标滚轮键触发）。
- 程序会读取“选中词/短语 + 所在句子”，先写入数据库，再请求英伟达 NIM 大模型生成题目。
- 题目支持三种类型：选择题（choice）、填空题（fill）、问答题（qa）。
- 卡片采用“等 AI 返回再弹出”的策略；超时或解析失败时自动降级为本地填空题，保证流程不中断。
- AI 回传机制已改为 `Future` 等待结果，不再依赖 Qt signal/slot，避免非 Qt 主事件循环下回调丢失。
- 运行中只使用模型正式回答 `content`；`reasoning_content` 不作为最终题目内容。
- `QuizTrigger` 新增可用配置：模型选择、超时时间、防抖间隔、是否回退本地题、reasoning 开关。
...
| `tokenize()` | 将句子分词为 token 列表（使用正则 `r"(\w+'\w+|\w+|[^\w\s])"`） |
| `is_word_token()` | 判断 token 是否为可选单词（包含字母） |
